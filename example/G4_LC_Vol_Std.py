...
def f3(indv, generation, PopName=G4input.ROOTName,Pop=0,Folder=G4input.RelativeFolder):
    f=PopName+str(generation)+"_"+str(indv)+"_0.root"
    inFile = ROOT . TFile . Open ( Folder+f ," READ ")
    tree = inFile . Get ("EndOfRun")
    tree.GetEntry(0)
...
nindv=100
problem = Problem(num_of_variables=10, objectives=[f1,f2], variables_range=[(0.5,1.75)], same_range=True, expand=False, obj_idx=True)
